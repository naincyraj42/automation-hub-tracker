...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (q)        query = query.or(`project.ilike.%${q}%,owner.ilike.%${q}%,tech.ilike.%${q}%`);
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabase
      .from('projects')
      .insert([{
        project: body.project, category: body.category, status: body.status,
        priority: body.priority || '', rag: body.rag || 'G',
        owner: body.owner || '', spoc: body.spoc || '', tech: body.tech || '',
        blocker: body.blocker || '', eta: body.eta || '',
        mc: body.mc || 'No', mc_reason: body.mc_reason || '',
